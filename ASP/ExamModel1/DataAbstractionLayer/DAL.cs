...
namespace ASP.DataAbstractionLayer
{
    public class DAL
    {
        ...
        public DAL()
        {
            string connectionString = "server=localhost;uid=root;pwd=;database=test2;";
        ...
        public Person Login(string username)
        {
            ...
            try
            {
                MySqlCommand command = new MySqlCommand();
                command.Connection = connection;
                command.CommandText = "select * from persons where name='" + username +"'";
                MySqlDataReader reader = command.ExecuteReader();

                if (reader.Read())
                {
                    person.id = reader.GetInt16("id");
                    person.name = reader.GetString("name");
                    person.age = reader.GetInt16("age");
